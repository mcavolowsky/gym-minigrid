...
class TaskDescriptor():
    def __init__(self, envD, rmDesc, rmOrder, rmSize = 3, observ=True, seed=10, time_steps=None):
        """
        ...
        @param rmDesc: the room descriptor array (self.roomDescriptor)
        @param rmOrder: the room order array (self.roomOrdering)
        @param rmSize: the room size (self.roomSize)
        @param observ: the observability (full or partial) (self.observability)
        @param seed: the seed (self.seed)
        """
        self.envDescriptor = np.array(envD)
        self.roomDescriptor = rmDesc
        self.roomOrdering = rmOrder
        self.roomSize = rmSize
        self.observability = observ
        self.seed = seed

        assert self.roomSize >= 3           # 3 is the effective minimum and will cause other issues
    ...
    def __str__(self):
        ...
        """
        return 'td'+''.join(str(e) for e in self.roomDescriptor) + '-' + \
                ''.join(str(e) for e in self.roomOrdering) + '-' + \
                str(self.roomSize) + '-' + \
               ('T' if self.observability else 'F')
...
def a_putDown(env, v): # should not get reward for putdown if it has never picked up anything
    # I modified this because there are cases where we can satisfy gotoDropoff and putDown
    # but we don't satisfy the top-leveldropOff (when we are in the room, but facing out and
    # drop the ball out of the room).
    if env.carrying == None:
        for item in env.obj:
            if not env.get_room(0,0).pos_inside(*item.cur_pos):
                return False
        return True
